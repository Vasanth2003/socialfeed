...
router.get("/all", async (req, res) => {
  try {
    ...
    if (sort === "likes") queryOptions.order = [["likes", "DESC"]];
    else if (sort === "oldest") queryOptions.order = [["createdAt", "ASC"]];
    
    if (user) queryOptions.where = { username: user };
...
router.post("/comment/:id", async (req, res) => {
  try {
    ...
    let existingComments = post.comments ? JSON.parse(post.comments) : [];
